...
def main(data, make_new=True):
    ...
    for stuck, code in tqdm(data): 
        ...
        g.run("create CONSTRAINT IF NOT EXISTS FOR (n: `主营构成`) REQUIRE (n.name, n.个股) IS Unique")
        try:
            basic_business = ak.stock_zygc_ym(symbol=code)
        except Exception as e:
            logging.error(f'主营构成 api获取异常：{code}，错误信息：{e}')
            continue
        for report_period in set(basic_business['报告期']):
            node_properties = {"name": report_period, "个股": stuck}
            try:
                g.create(Node("主营构成",**node_properties))
            except:
                pass
            g.run(f"match (n:`个股`), (m:`主营构成`) where n.name='{stuck}' and m.个股='{stuck}' and \
                    m.name='{report_period}' merge (n)-[r:`基本面`]->(m)")
            df_data = basic_business[basic_business['报告期']==report_period]
            attr_names = [i for i in basic_business.columns[3:]]
            for rel_name in set(df_data['分类方向']):
                data_val = df_data[df_data['分类方向']==rel_name].iloc[:, 2:].values
                father_name = ''
                for d in data_val:
                    ...
                    if len(child_name)>0: 
                        ...
                        g.run(f"match (m:`主营构成`), (p:`主营指标`) where m.name='{report_period}' and m.个股='{stuck}'\
