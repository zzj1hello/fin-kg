...
def main(make_new=True):
    ...
    # 打开csv文件
    with open('../行业数据\\研报/行业研报基本信息.csv', encoding='utf-8') as csvfile:
    ...
    g.run("CREATE CONSTRAINT FOR (n:`机构`) REQUIRE n.name IS UNIQUE")
    g.run("create CONSTRAINT FOR (n: `研报`) REQUIRE n.name IS UNIQUE")
    g.run("create CONSTRAINT FOR (n: `行业`) REQUIRE n.name IS UNIQUE")
