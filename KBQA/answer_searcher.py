...
class AnswerSearcher:
    ...
    def search_main(self, question, ent_dict):
        ...
        for i in ent_dict['主体']['股票'] + ent_dict['主体']['行业']:
            if i:
                tmp_cnt += 1
        if tmp_cnt > 1 or len(ent_dict['发布时间'])>1:
            self.is_trunc = True
        ...
        for tp, subjects in ent_dict['主体'].items():
            ...
            for subject in subjects:
                if subject:
                    ...
                    times[subject] = time[0]['time']

        sql_dict = self.QP.question2sql(question, ent_dict, times)
            ...
            if res:
                ...
                for r in res:
                    ...
                    if labl in self.rel_triple_hlt['财务指标']:
                        tmp_output += write_prop_tabular_fin(labl, prop, no_rep, self.is_trunc)
                        tmp_output_trunc += write_prop_tabular_fin(labl, prop, no_rep, True)
                    else:
                        tmp_output += write_prop_tabular(labl, prop, no_rep, self.is_trunc)
                        tmp_output_trunc += write_prop_tabular(labl, prop, no_rep, True)
            ...
            if res:
                ...
                for r in res:
                    ...
                    if labl_node in [i[-1] for i in self.rel_triple['包含']]:
                        tmp_output += write_prop_tabular_fin(labl_node, prop_node, no_rep, self.is_trunc)
                        tmp_output_trunc += write_prop_tabular_fin(labl_node, prop_node, no_rep, True)
                    else:
                        tmp_output += write_prop_tabular(labl_node, prop_node, no_rep, self.is_trunc)
